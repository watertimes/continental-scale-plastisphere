###figure 5 the neutral model for the soil and plastisphere community
...
#Output 3 statistical result data tables(p.csv,freq.csv,freq.pred.csv）
...
inter.col[bacnlsALL$freq <= bacnlsALL$Lower]<-'#A52A2A'#Occurs less frequently than predicted by the neutral community model
inter.col[bacnlsALL$freq >= bacnlsALL$Upper]<-'#29A6A6'#Occurs more frequently than predicted by the neutral community model
library(grid)
grid.newpage()
